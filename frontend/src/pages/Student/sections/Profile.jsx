// src/pages/Student/Profile.js (Student Profile - Updated)
...
const StudentProfile = () => {
  ...
  const summaryCards = useMemo(() => (
    [
      ...
      {
        label: "Current Grade",
        value: profile?.currentGradeLevel ? `Grade ${profile.currentGradeLevel}` : "Unassigned",
        helper: profile?.section ? `Section ${profile.section}` : "Based on enrolled subjects",
            ...
            <div className="max-w-2xl space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-white/85">
                  Review personal details, see your current academic level, and keep your contact information current.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="rounded-3xl border border-gray-200 bg-white/95 p-6 shadow-xl ring-1 ring-black/5">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Update your contact details while academic information is automatically managed.</p>
          </div>
          {!isEditing ? (
            <button
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          ...
          {/* Current Grade - Non-editable */}
          <div className="space-y-2 text-sm">
            <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">
              <FontAwesomeIcon icon={faGraduationCap} className="mr-2 text-gray-400" /> Current Grade
            </span>
            <div className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-2 text-sm font-medium text-gray-800">
              <div className="flex items-center justify-between">
                <span>{profile?.currentGradeLevel ? `Grade ${profile.currentGradeLevel}` : "Unassigned"}</span>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {profile?.section ? `Section ${profile.section}` : "Based on your enrolled subjects"}
            </p>
          </div>
          ...
          <div className="space-y-2 text-sm">
            <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">
              <FontAwesomeIcon icon={faIdBadge} className="mr-2 text-gray-400" /> Learner Reference Number
            </span>
            <p className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-2 text-sm font-medium text-gray-800">
              {profile?.lrn || "Unassigned"}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              Official DepEd LRN • Managed by school administration
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
          ...
          <strong>Academic information:</strong> Grade level is automatically calculated from your enrolled subjects. Section and LRN are managed by school administration.
        </div>
      </section>
