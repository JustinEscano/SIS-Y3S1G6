...
const StudentSubjectList = () => {
  ...
  const handleCardClick = (subjectId) => {
    // Navigate to analytics for both active and archived subjects
    // The analytics page should handle read-only mode for archived subjects
    navigate(`/student/subjects/${subjectId}/analytics`, { 
      state: { isArchived: viewMode === 'archived' } 
    });
  };
  ...
  const summaryCards = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: subjects.length,
        helper: 'Tap a card to open insights',
        icon: faChartLine,
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              {viewMode === 'active'
                ...
                : 'Review your completed classes with historical analytics.'}
            </p>
          </div>
        </div>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {paginatedSubjects.map((subject) => (
                <div
                  ...
                  className="group relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition cursor-pointer hover:-translate-y-1 hover:shadow-lg"
                >
