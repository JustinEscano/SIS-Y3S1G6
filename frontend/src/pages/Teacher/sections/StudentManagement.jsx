...
const StudentManagement = () => {
    ...
    const [editFormData, setEditFormData] = useState({ 
        ...
        lrn: '', 
        section: '' 
        // Removed currentGradeLevel from edit form
    });
    ...
    const studentSummary = useMemo(() => {
        ...
        // Calculate active students based on currentGradeLevel (automatically calculated)
        ...
        // Get unique grade levels from automatically calculated currentGradeLevel
        const gradeLevels = Array.from(new Set(
            students
                .map((student) => student.currentGradeLevel)
                .filter(Boolean)
        )).sort((a, b) => a - b);
    ...
    const fetchStudents = async () => {
        ...
        try {
            ...
            console.log('📋 Extracted students with automatic grade levels:', fetchedStudents.length);
    ...
    const openEditStudentModal = (student) => {
        ...
        setEditFormData({
            ...
            // Note: currentGradeLevel is not included as it's automatic
        });
        setEditError('');
    ...
    const handleUpdateStudent = async () => {
        ...
        try {
            ...
            // Only send editable fields - currentGradeLevel is automatic
            const payload = {
                ...editFormData
            };
    ...
    const filteredStudents = useMemo(() => {
        ...
        if (!lowerSearchTerm) return students;
    ...
    const summaryCards = [
        ...
        },
        {
            ...
            helper: 'Automatically assigned to current grade',
            icon: faUserCheck,
        ...
        },
        {
            ...
            helper: 'No current grade level assigned',
            icon: faUserSlash,
    ...
    const displayedGrades = gradeBadges.slice(0, 6);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-gray-100 px-4 pb-16 pt-10 sm:px-8">
                        ...
                        <div className="max-w-2xl space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-sm text-white/85">
                                    Search, review, and track every student across your classes. Grade levels are automatically calculated based on enrolled subjects.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <section className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div className="rounded-2xl border border-gray-200 bg-white/95 p-5 shadow-sm">
                    <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-gray-600">
                        <FontAwesomeIcon icon={faLayerGroup} /> Current grade levels
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    {hiddenGradeCount > 0 && (
                        <div className="mt-3 text-xs text-gray-500">
                            +{hiddenGradeCount} more grade levels
                        </div>
                    )}
                    <div className="mt-2 flex items-center gap-1 text-xs text-blue-600">
                        <FontAwesomeIcon icon={faInfoCircle} className="text-xs" />
                        <span>Grade levels are automatically calculated</span>
                    </div>
                </div>
            </section>
                    ...
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-500">
                                Showing {paginatedStudents.length} of {filteredStudents.length} students filtered.
                                <span className="ml-2 mt-2 text-blue-600 flex items-center gap-2">
                                    <FontAwesomeIcon icon={faInfoCircle} className="text-xs" />
                                    Grade levels are automatically calculated from enrolled subjects
                                </span>
                            </p>
                        </div>
                                                ...
                                                <td className="px-6 py-4 text-sm font-medium text-gray-700">
                                                    {student.currentGradeLevel ? (
                                                        <div className="flex items-center gap-2">
                                                            <span>Grade {student.currentGradeLevel}</span>
                                                            <FontAwesomeIcon icon={faInfoCircle} className="text-blue-500 text-xs" title="Automatically calculated from enrolled subjects" />
                                                        </div>
                                                    ) : (
                                                        <span className="italic text-gray-400">No active subjects</span>
                                                    )}
                                                </td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl border border-gray-100 bg-white p-6 shadow-2xl">
                        ...
                        <div className="space-y-3 text-sm text-gray-700">
                            ...
                            <div>
                                <span className="font-medium text-gray-500">Current grade</span>
                                <div className="flex items-center gap-2">
                                    <p className="text-gray-900">
                                        {selectedStudent.currentGradeLevel ? `Grade ${selectedStudent.currentGradeLevel}` : <span className="italic text-gray-500">No active subjects</span>}
                                    </p>
            ...
            {/* Edit Student Modal - Note: currentGradeLevel is not editable */}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl border border-gray-100 bg-white p-6 shadow-2xl">
                        ...
                        <div className="space-y-3 text-sm text-gray-700">
                            ...
                            {/* Current Grade Level - Display Only */}
                            <div>
                                <label className="text-xs font-semibold uppercase tracking-wide text-gray-500">Current grade level</label>
                                <div className="mt-1 rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5">
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm text-gray-800">
                                            {selectedStudent.currentGradeLevel ? `Grade ${selectedStudent.currentGradeLevel}` : 'No active subjects'}
                                        </span>
                                        <FontAwesomeIcon icon={faInfoCircle} className="text-blue-500 text-xs" title="Automatically calculated from enrolled subjects" />
                                    </div>
                                </div>
                                <p className="mt-1 text-xs text-gray-500">
                                    Grade level is automatically determined by enrolled subjects
                                </p>
                            </div>
                        </div>
